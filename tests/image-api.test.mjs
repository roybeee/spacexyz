...
await test('project save rejects foreign or incorrect-kind images in every inactive location',async()=>{
 const original=assets.get(imageId);const make=where=>{let s=m.initialScene();if(where==='surface')s.room.surfaces.floor.finish={textureId:imageId};else if(where==='node'){s.nodes[0].hidden=true;s.nodes[0].finish={textureId:imageId};}else if(where==='photo')s.nodes[0].objectPhoto={imageId,representation:'parametric',analysis:'manual',dimensions:'entered',brand:'',productCode:'',notes:[]};else if(where==='face')s.nodes[0].faceFinishes={'unused:0':{textureId:imageId}};else if(where==='underlay')s.underlay={imageId,name:'도면',pixelWidth:4,pixelHeight:4,width:5000,x:0,z:0,rotation:0,opacity:.5,visible:false};else{s.facade=m.defaultFacade(s);s.facade.sign.enabled=false;s.facade.sign.logoId=imageId;}s=m.saveVariant(s,'stored');s.nodes=[];s.room.surfaces.floor.finish={};delete s.facade;delete s.underlay;return s};
 for(const where of ['surface','node','face','logo','underlay','photo']){const s=make(where),before=saves;original.owner='owner-b';assert.equal((await m.save(saveReq(s))).status,400);original.owner='owner-a';original.kind='model';assert.equal((await m.save(saveReq(s))).status,400);assert.equal(saves,before);original.kind='image';assert.equal((await m.save(saveReq(s))).status,200);}
});
