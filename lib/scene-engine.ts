...
import { materials, type MaterialId, type SceneData, type SceneNode, type Selection } from './scene-model';
...
export class SceneEngine {
    ...
    constructor(public host: HTMLElement, callbacks: {
        ...
        this.camera = new T.PerspectiveCamera(38, 1, .05, 100);
        ...
        this.transform.addEventListener('dragging-changed', e => { this.dragActive = !!e.value; this.orbit.enabled = !e.value; if (e.value)
            this.suppress = true; });
        this.transform.addEventListener('mouseUp', () => { const o = this.transform.object; if (!o)
            return; const id = o.userData.nodeId as string; const n = this.data?.nodes.find(n => n.id === id); if (!n)
            return; const patch: Partial<SceneNode> = { x: Math.round(o.position.x * 1000), y: Math.max(0, Math.round(o.position.y * 1000)), z: Math.round(o.position.z * 1000), rotation: Math.round(Math.atan2(2 * (o.quaternion.w * o.quaternion.y + o.quaternion.x * o.quaternion.z), 1 - 2 * (o.quaternion.y * o.quaternion.y + o.quaternion.z * o.quaternion.z)) * 180 / Math.PI) }; if (patch.x !== n.x || patch.y !== n.y || patch.z !== n.z || patch.rotation !== n.rotation)
            this.onTransform(id, patch); });
        this.pointerDown = e => { this.down = [e.clientX, e.clientY]; this.suppress = this.dragActive; };
        this.pointerUp = e => { if (this.suppress || this.dragActive || Math.hypot(e.clientX - this.down[0], e.clientY - this.down[1]) > 5)
            return; const r = this.renderer.domElement.getBoundingClientRect(); const ray = new T.Raycaster(); ray.setFromCamera(new T.Vector2((e.clientX - r.left) / r.width * 2 - 1, -((e.clientY - r.top) / r.height) * 2 + 1), this.camera); if (this.mode === 'draw') {
            ...
            return;
        } const hits = ray.intersectObjects(this.root.children, true).filter(h => { let o: T.Object3D | null = h.object; while (o) {
            if (!o.visible)
                return false;
            o = o.parent;
        } return !!h.object.userData.nodeId; }); const hit = hits[0]; if (!hit) {
            this.onSelect(null);
            return;
        } const id = hit.object.userData.nodeId; const part = hit.object.userData.part as string; this.onSelect({ id, face: this.selectionMode === 'face' && part ? `${part}:${hit.face?.materialIndex ?? 0}` : undefined }); };
    ...
    private resize() { const w = this.host.clientWidth, h = this.host.clientHeight; if (!w || !h)
        return; this.renderer.setSize(w, h, false); this.camera.aspect = w / h; this.camera.updateProjectionMatrix(); }
    ...
    texture(id: MaterialId) { const existing = this.textures.get(id); if (existing)
        return existing; const m = materials.find(m => m.id === id)!; const canvas = document.createElement('canvas'); canvas.width = canvas.height = 256; const c = canvas.getContext('2d')!; c.fillStyle = m.color; c.fillRect(0, 0, 256, 256); let seed = 43; const random = () => { seed = (seed * 1664525 + 1013904223) >>> 0; return seed / 4294967296; }; if (m.pattern === 'wood') {
        for (let i = 0; i < 240; i++) {
    ...
    } const tx = new T.CanvasTexture(canvas); tx.colorSpace = T.SRGBColorSpace; tx.wrapS = tx.wrapT = T.RepeatWrapping; tx.anisotropy = 4; this.textures.set(id, tx); return tx; }
    private material(id: MaterialId, color?: string, repeat: [
        number,
        number
    ] = [1, 1]) { const m = materials.find(m => m.id === id)!; const map = this.texture(id).clone(); map.needsUpdate = true; map.repeat.set(...repeat); const material = new T.MeshStandardMaterial({ color: color ?? '#ffffff', map, roughness: m.roughness, metalness: m.metalness, side: T.DoubleSide }); if (id === 'glass') {
        material.transparent = true;
        material.opacity = .28;
        material.depthWrite = false;
    } return material; }
    private box(parent: T.Object3D, w: number, h: number, d: number, x: number, y: number, z: number, id: string, part: string, mat: MaterialId, node?: SceneNode): T.Mesh<T.BoxGeometry, T.Material | T.Material[]> { const geometry = new T.BoxGeometry(Math.max(.006, w), Math.max(.006, h), Math.max(.006, d)); const sizes: [
        [
            number,
            number
        ],
    ...
    ] = [[d, h], [d, h], [w, d], [w, d], [w, h], [w, h]]; const mats = sizes.map((size, i) => this.material(node?.faces[`${part}:${i}`] ?? mat, node?.color, [Math.max(.1, size[0] / .9), Math.max(.1, size[1] / .9)])); const mesh = new T.Mesh(geometry, mats); mesh.position.set(x, y, z); mesh.castShadow = true; mesh.receiveShadow = true; mesh.userData = { nodeId: id, part }; parent.add(mesh); return mesh; }
    private cylinder(parent: T.Object3D, r: number, h: number, x: number, y: number, z: number, id: string, part: string, mat: MaterialId, node?: SceneNode, rTop?: number) { const mats = [0, 1, 2].map(i => this.material(node?.faces[`${part}:${i}`] ?? mat, node?.color)); const mesh = new T.Mesh(new T.CylinderGeometry(rTop ?? r, r, h, 32), mats); mesh.position.set(x, y, z); mesh.castShadow = true; mesh.receiveShadow = true; mesh.userData = { nodeId: id, part }; parent.add(mesh); return mesh; }
    ...
    private surfaceMaterial(id: keyof SceneData['room']['surfaces'], repeat: [
        number,
        number
    ]) { const s = this.data!.room.surfaces[id]; return this.material(s.material, s.color, repeat); }
    private buildNode(n: SceneNode) {
        ...
        switch (n.kind) {
            ...
            case 'plant': {
                ...
                for (let i = 0; i < 10; i++) {
                    const angle = i * 2.4;
                    const mesh = new T.Mesh(new T.SphereGeometry(1, 12, 8), this.material(n.faces[`leaf${i}:0`] ?? n.material, n.color));
            ...
            case 'pendant': {
                cylinder(w / 2, h * .6, 0, h * .3, 0, 'shade', n.material, w * .22);
    ...
    private updateCutaway() { if (!this.data)
        return; const p = this.camera.position; const d = this.data.room.depth / 1000, w = this.data.room.width / 1000; for (const item of this.root.children) {
    ...
    setSelection(selection: Selection) { this.selection = selection; this.transform.detach(); if (this.outline) {
        this.helpers.remove(this.outline);
        this.outline.geometry.dispose();
        (this.outline.material as T.Material).dispose();
        this.outline = undefined;
    } if (!selection)
        return; const n = this.data?.nodes.find(n => n.id === selection.id); const obj = this.root.children.find(o => o.userData.nodeId === selection.id); if (!obj)
        return; this.selectedObject = obj; if (selection.face && !(selection.id in this.data!.room.surfaces)) {
        const part = selection.face.split(':')[0];
        let partObject: T.Object3D | undefined;
        obj.traverse(o => { if (o.userData.part === part && !partObject)
            partObject = o; });
        this.outline = new T.BoxHelper(partObject ?? obj, 0x7861f0);
    }
    else
        this.outline = new T.BoxHelper(obj, 0x7861f0); (this.outline.material as T.Material).depthTest = false; this.outline.renderOrder = 10; this.helpers.add(this.outline); if (n && !n.locked && !n.hidden && !n.host && (this.mode === 'translate' || this.mode === 'rotate')) {
    ...
    setView(view: ViewMode) { this.view = view; const width = (this.data?.room.width ?? 7200) / 1000, depth = (this.data?.room.depth ?? 6400) / 1000; const size = Math.max(width, depth); this.orbit.target.set(0, .55, 0); this.orbit.enableRotate = view !== 'top'; this.orbit.maxPolarAngle = view === 'interior' ? Math.PI * .58 : Math.PI * .49; if (view === 'perspective')
        this.camera.position.set(size * 1.25, size * 1.12, size * 1.5); if (view === 'top') {
        this.camera.position.set(0, size * 2.05, .001);
        this.orbit.target.set(0, 0, 0);
    } if (view === 'front')
        this.camera.position.set(0, 2.9, size * 1.6); if (view === 'interior') {
        this.camera.position.set(-width * .25, 1.65, depth * .4);
        this.orbit.target.set(0, 1.35, -depth * .4);
    } this.orbit.update(); }
    zoom(factor: number) { this.camera.position.sub(this.orbit.target).multiplyScalar(factor).add(this.orbit.target); this.orbit.update(); }
    captureCamera() { return { position: this.camera.position.toArray() as [
            number,
            number,
            number
    ...
    }) { this.camera.position.fromArray(camera.position); this.orbit.target.fromArray(camera.target); this.orbit.enableRotate = true; this.view = 'perspective'; this.orbit.update(); }
    async exportGlb() { const clone = this.root.clone(true); clone.traverse(o => { if (o.userData.wall || o.userData.host)
        o.visible = !this.data?.nodes.find(n => n.id === o.userData.nodeId)?.hidden; }); return await new GLTFExporter().parseAsync(clone, { binary: true, onlyVisible: true, maxTextureSize: 1024 }) as ArrayBuffer; }
    async screenshot() { const oldSize = new T.Vector2(); this.renderer.getSize(oldSize); const oldRatio = this.renderer.getPixelRatio(); const oldAspect = this.camera.aspect; const helperVis = this.helpers.visible; const gizmoVis = this.transform.getHelper().visible; this.helpers.visible = false; this.transform.getHelper().visible = false; try {
    ...
    private disposeMaterial(material: T.Material | T.Material[]) { for (const m of Array.isArray(material) ? material : [material]) {
        if (m instanceof T.MeshStandardMaterial)
            m.map?.dispose();
        m.dispose();
    } }
    private clearGroup(group: T.Group) { group.traverse(o => { if (o instanceof T.Mesh) {
        o.geometry.dispose();
        this.disposeMaterial(o.material);
    } }); group.clear(); }
    dispose() { cancelAnimationFrame(this.frame); this.observer.disconnect(); this.renderer.domElement.removeEventListener('pointerdown', this.pointerDown); this.renderer.domElement.removeEventListener('pointerup', this.pointerUp); this.orbit.dispose(); this.transform.dispose(); this.clearGroup(this.root); this.grid.geometry.dispose(); (this.grid.material as T.Material).dispose(); if (this.outline) {
        this.outline.geometry.dispose();
        (this.outline.material as T.Material).dispose();
    } for (const tx of this.textures.values())
        tx.dispose(); this.environment.dispose(); this.renderer.dispose(); this.renderer.domElement.remove(); }
}
