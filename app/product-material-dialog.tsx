...
export default function ProductMaterialDialog({currentId,target,onClose,onApply}:{currentId?:string|null;target:string;onClose:()=>void;onApply:(p:MaterialProduct)=>Promise<boolean>}){
 ...
 return <Dialog open onOpenChange={o=>!o&&!busy&&onClose()}><DialogContent className="product-material-dialog" showCloseButton={!busy}>
  ...
  <div className="product-library-layout"><section className="product-browser">
   ...
   {scope==='colors'&&<p className="product-source-summary">삼화페인트 · 950 / 600 / 1200 / 기타 컬러북</p>}
   <div className="product-search"><Search size={18}/><input autoFocus aria-label="브랜드 제품 코드 검색" placeholder="제품 코드, 소재명, 시트지·바닥재·석재 검색" value={query} onChange={e=>{setQuery(e.target.value);setPage(0)}} disabled={busy}/></div>
   <div className="product-filter"><label>브랜드<select aria-label="마감재 브랜드" value={brand} onChange={e=>{setBrand(e.target.value);setBook('전체');setPage(0);if(e.target.value==='삼화페인트'){setScope('colors');setProductType('전체');setCategory('전체')}else if(e.target.value!=='전체'&&scope==='colors'){setScope('products');setProductType('전체');setCategory('전체')}}} disabled={busy}>{['전체',...brands].map(b=><option key={b}>{b}</option>)}</select></label>{scope!=='colors'&&<label>제품군<select aria-label="마감재 제품군" value={productType} onChange={e=>{setProductType(e.target.value);setPage(0)}} disabled={busy}>{['전체',...productTypes].map(t=><option key={t}>{t}</option>)}</select></label>}<label>무늬·소재<select aria-label="마감재 무늬와 소재" value={category} onChange={e=>{setCategory(e.target.value);setPage(0)}} disabled={busy}>{['전체',...categories].map(c=><option key={c}>{c}</option>)}</select></label></div>
   ...
   <p className="product-results" role="status">검색 {filtered.length.toLocaleString()}개 <span>{filtered.length?`${safePage*PAGE_SIZE+1}–${Math.min((safePage+1)*PAGE_SIZE,filtered.length)} 표시`:'0개 표시'} · {new Set(filtered.map(p=>p.brand)).size}개 브랜드</span></p>
