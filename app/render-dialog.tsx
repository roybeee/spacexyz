...
export default function RenderDialog({ open, onClose, scene, photoUrl, apiKey, aiReady, onConnect, capture, onResult }: {
    open: boolean;
    onClose: () => void;
    scene: SceneData;
    photoUrl: string;
    ...
    async function generate() { if (flight.current || !image)
        ...
        const response = await fetch('/api/render', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ requestId: crypto.randomUUID(), name: `${scene.name.slice(0, 80)} · AI 시안`, prompt, quality, image: inputImage, reference: ref, apiKey: apiKey || undefined }) });
 ...
 {tab === 'create' && <div className="render-create"><div><div className="render-preview">{image ? <img src={image} alt="시안 생성에 사용할 3D 장면"/> : <div className="render-placeholder"><Camera /><p>현재 장면 준비 중</p></div>}{capturing && <div className="render-loading"><LoaderCircle className="spin"/>장면 불러오는 중</div>}</div><div className="render-preview-label"><b>배치 기준 · 현재 3D 장면</b><button className="text-button" onClick={refresh} disabled={capturing || busy}><RefreshCw size={14}/>다시 캡처</button></div><p className="fineprint">원하는 각도를 3D 화면에서 먼저 정하세요. 실내 보기에서 매장 내부를 촬영하면 공간의 분위기가 잘 드러납니다.</p>{reference && <div className="render-reference"><img src={photoUrl} alt="소재와 분위기 참고 사진"/><span>분위기 참고 사진<small>{scene.photoId ? '내 매장 사진' : '예시 카페 사진'}</small></span></div>}</div><div className="render-options"><label className="switch-row"><span>참고 사진 분위기 반영</span><Switch checked={reference} onCheckedChange={setReference} disabled={busy}/></label><label className="field-label">원하는 분위기<textarea className="text-input" rows={5} maxLength={1500} value={prompt} onChange={e => setPrompt(e.target.value)} disabled={busy}/></label><div className="render-presets">{['내추럴 우드와 밝은 벽, 부드러운 낮빛', '브러시드 스틸과 우드, 선명한 소재 대비', '크림 벽과 딥 네이비 포인트, 따뜻한 조명'].map(p => <button key={p} disabled={busy} onClick={() => setPrompt(p)}>{p}</button>)}</div><label className="field-label">이미지 품질</label><Tabs value={quality} onValueChange={v => { if (!busy)
