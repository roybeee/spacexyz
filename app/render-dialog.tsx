...
type RenderAttempt={requestId:string;name:string;prompt:string;quality:string;view:'interior'|'exterior';signText?:string;image:string;reference?:string};
export default function RenderDialog({ open, onClose, scene, view, photoUrl, apiKey, aiReady, onConnect, capture, onResult }: {
    ...
    const [attempt,setAttempt]=useState<RenderAttempt|null>(null);
    ...
    useEffect(() => { if (open) {
        if(!attemptRef.current)refresh();
        loadGallery();
    }
    else
        captureEpoch.current++; }, [open,view]);
    async function refresh() { if(attemptRef.current)return; const token = ++captureEpoch.current; setCapturing(true); setImage(''); setError(''); try {
    ...
    async function generate() {
        if(flight.current||!image)return;
        if(!attemptRef.current&&!aiReady&&!apiKey){onConnect();return;}
        flight.current=true;setBusy(true);setError('');
        try {
            let job=attemptRef.current;
            if(!job){
                const ref=reference?(await photoData(photoUrl)).image:undefined;
                if(!mounted.current)return;
                job={requestId:randomId(),name:`${scene.name.slice(0,80)} · ${view==='exterior'?'외관':'실내'} AI 시안`,prompt,quality,view,signText:view==='exterior'&&scene.facade?.sign.enabled&&!scene.facade.sign.logoId?scene.facade.sign.text:undefined,image,reference:ref};
                attemptRef.current=job;setAttempt(job);
            }
            const response=await fetch('/api/render',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...job,apiKey:apiKey||undefined})});
            const data=await response.json() as {error?:string;result?:RenderResult};
            if(!response.ok||!data.result)throw new Error(data.error||'시안 결과를 확인하지 못했습니다. 같은 요청으로 다시 확인해 주세요.');
            if(!mounted.current)return;
            setResult(data.result);setTab('result');
            setGallery(prev=>[data.result!,...prev.filter(item=>item.id!==data.result!.id)]);
            attemptRef.current=null;setAttempt(null);
            onResult(data.result);
        } catch(e){if(mounted.current)setError(e instanceof Error?e.message:'결과를 확인하지 못했습니다.');}
        finally{flight.current=false;if(mounted.current)setBusy(false);}
    }
    function startNewRequest(){if(flight.current)return;attemptRef.current=null;setAttempt(null);setError('');void refresh();}
    ...
    return <Dialog open={open} onOpenChange={o => { if (!o && !busy && !capturing)
        onClose(); }}><DialogContent className="render-dialog" showCloseButton={!busy&&!capturing}><DialogHeader><DialogTitle>AI 컨셉 이미지</DialogTitle><DialogDescription>현재 3D 배치를 기준으로 소재와 빛을 실사 느낌으로 표현합니다.</DialogDescription></DialogHeader><Tabs value={tab} onValueChange={v => { if (!busy)
 ...
 {tab === 'create' && <div className="render-create"><div><div className="render-preview">{image ? <img src={image} alt="시안 생성에 사용할 3D 장면"/> : <div className="render-placeholder"><Camera /><p>{capturing?'현재 장면 준비 중':'장면을 캡처하지 못했습니다'}</p></div>}{capturing && <div className="render-loading"><LoaderCircle className="spin"/>장면 불러오는 중</div>}</div><div className="render-preview-label"><b>배치 기준 · {view==='exterior'?'매장 외관':'현재 3D 장면'}</b><button className="text-button" onClick={refresh} disabled={capturing || busy || !!attempt}><RefreshCw size={14}/>다시 캡처</button></div><p className="fineprint">{view==='exterior'?'간판·어닝·유리 전면의 크기와 위치를 유지하도록 요청합니다. 간판 문구는 원본과 비교해 확인하세요.':'원하는 각도를 3D 화면에서 먼저 정하세요. 실내 보기에서 매장 내부를 촬영하면 공간의 분위기가 잘 드러납니다.'}</p>{reference && <div className="render-reference"><img src={photoUrl} alt="소재와 분위기 참고 사진"/><span>분위기 참고 사진<small>{scene.photoId ? '내 매장 사진' : '예시 카페 사진'}</small></span></div>}</div><div className="render-options"><label className="switch-row"><span>참고 사진 분위기 반영</span><Switch checked={reference} onCheckedChange={setReference} disabled={busy||!!attempt}/></label><label className="field-label">원하는 분위기<textarea className="text-input" rows={5} maxLength={1500} value={prompt} onChange={e => setPrompt(e.target.value)} disabled={busy||!!attempt}/></label><div className="render-presets">{['내추럴 우드와 밝은 벽, 부드러운 낮빛', '브러시드 스틸과 우드, 선명한 소재 대비', '크림 벽과 딥 네이비 포인트, 따뜻한 조명'].map(p => <button key={p} disabled={busy||!!attempt} onClick={() => setPrompt(p)}>{p}</button>)}</div><label className="field-label">이미지 품질</label><Tabs value={quality} onValueChange={v => { if (!busy&&!attempt)
        setQuality(v); }}><TabsList className="full"><TabsTrigger value="medium">기본</TabsTrigger><TabsTrigger value="high">고품질</TabsTrigger></TabsList></Tabs><p className="fineprint">생성에 수 분이 걸릴 수 있습니다. 사진과 장면을 OpenAI에 전송하며, 고품질은 API 비용과 시간이 더 들 수 있습니다.</p><button className="primary-button full" disabled={!image || busy || capturing} onClick={generate}>{busy ? <LoaderCircle className="spin" size={16}/> : <Sparkles size={16}/>} {busy ? '시안 요청 확인 중…' : attempt ? '같은 요청 결과 확인' : aiReady || apiKey ? 'AI 시안 만들기' : 'AI 연결하고 시작'}</button>{attempt&&!busy&&<div><p className="fineprint">이 요청의 장면과 설정을 유지하고 있습니다. 결과 확인은 같은 요청 번호를 사용합니다. 새 요청으로 시작하면 별도 생성 비용이 발생할 수 있습니다.</p><button className="outline-button full" onClick={startNewRequest}>새 요청으로 시작</button></div>}{busy && <p role="status" className="render-wait">소재·빛·그림자를 표현하고 있습니다. 이 창을 유지해 주세요.</p>}</div></div>}
