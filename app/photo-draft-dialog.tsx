...
export default function PhotoDraftDialog({ open, onClose, photoUrl, scene, apiKey, aiReady, onReady, onConnect }: {
    ...
    async function generate(ai: boolean) { setError(''); let input: DraftInput; try {
        ...
        if (ai) {
            ...
            const r = await fetch('/api/photo-draft', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ input, image: p.image, apiKey: apiKey || undefined }) });
