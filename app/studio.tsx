'use client';
import {Pencil,Footprints} from 'lucide-react';
...
type Proposal = {
    ...
    title: string;
    details: string;
    source: 'AI' | '빠른 명령' | '스타일' | '사진 팔레트' | '3D 초안' | '디자인 안' | '외관' | '가구 세트' | '배열·정렬';
    baseScene?:string;
    addedIds?:string[];
...
export default function InteriorStudio() {
    ...
    function resetProjectView(next:SceneData){engine.current?.cancelMeasurement();setTool('select');setMeasureStarted(false);setMeasureError('');setEditingGroupId(null);engine.current?.setScene(next);selectView('perspective');setMultiSelect(false);}
