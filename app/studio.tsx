...
export default function InteriorStudio() {
    ...
    const doUndo = useCallback(() => {
        ...
        setEditingGroupId(null);engine.current?.cancelTransform();sceneRef.current=undo[undo.length-1];setScene(undo[undo.length - 1]);
    ...
    const doRedo = useCallback(() => {
        ...
        setEditingGroupId(null);engine.current?.cancelTransform();sceneRef.current=redo[redo.length-1];setScene(redo[redo.length - 1]);
    ...
    }, [doUndo, doRedo, removeSelected, modal, mobileLibrary, mobileInspector, editingGroupId, selection, proposal]);
    ...
    function resetProjectView(next:SceneData){setEditingGroupId(null);engine.current?.setScene(next);selectView('perspective');setMultiSelect(false);}
 ...
 <PlacementDialog open={modal === 'placement'} onClose={() => setModal(null)} scene={scene} selection={selection} onCommit={commit} onSelect={selectItem}/>
