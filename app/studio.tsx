...
export default function InteriorStudio() {
    ...
    async function captureForRender() { if (!engine.current)
        ...
        throw new Error('평면 보기에서 3D 또는 실내 보기로 바꾼 뒤 시안을 만드세요.'); const snapshot = { session: projectSession.current, scene: JSON.stringify(sceneRef.current) }; let image = ''; for (const width of [1536, 1024, 768]) {
 ...
 <RenderDialog open={modal === 'render'} onClose={() => setModal(null)} scene={scene} photoUrl={photoUrl} apiKey={apiKey} aiReady={aiReady} onConnect={() => { setNextModal('render'); setModal('settings'); }} capture={captureForRender} onResult={r => { if (renderSnapshot.current.session !== projectSession.current || renderSnapshot.current.scene !== JSON.stringify(sceneRef.current)) {
