...
export default function InteriorStudio() {
    ...
    const selectedMaterial = appearance?.material;
    ...
    const updateNode = useCallback((id: string, p: Partial<SceneNode>) => { const s = sceneRef.current, n = s.nodes.find(n => n.id === id); if (!n)
        return false; if (n.locked && Object.keys(p).some(k => k !== 'locked' && k !== 'hidden')) {
        toast.error('잠금을 해제한 뒤 수정하세요.');
        engine.current?.setScene(s);
        return false;
    } return commit({ ...s, nodes: s.nodes.map(n => n.id === id ? { ...n, ...p } : n) }); }, [commit]);
    ...
    function paint(id: MaterialId) { try {
    ...
    const inspectorContent = <><div className="inspector-title"><span className="eyebrow">PROPERTIES</span><h2>{currentName}</h2><span className="selection-label">{node ? kindNames[node.kind] : surface ? '공간 표면' : '선택한 요소 없음'}{selection?.face && faceMode === 'face' ? ' · 선택한 면' : ''}</span>{node?.estimated && <span className="estimate-label">초안 치수 · 현장 확인 필요</span>}</div>{selection && <><div className="property-section"><div className="small-heading"><span>선택 범위</span></div><Tabs value={faceMode} onValueChange={v => {
                ...
                }}><span className={`swatch pattern-${material.pattern}`} style={{ backgroundColor: material.color }}/><div><small>현재 소재</small><b>{material.name}</b></div><ChevronRight size={17}/></button>}{appearance && <MaterialDetail material={appearance.material} finish={appearance.finish} disabled={node?.locked} onChange={finish => { try {
