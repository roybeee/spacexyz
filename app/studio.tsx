'use client';
import BrandStandardsDialog from './brand-standards-dialog';
import {assertBrandSnapshot} from '@/lib/brand-standards';
import PhotoProductsDialog from './photo-products-dialog';
import HermesConnection from './hermes-connection';
import {aiFetch} from '@/lib/ai-client';
import {photoDimensionsMeasured} from '@/lib/object-photo-schema';
import {randomId} from '@/lib/random-id';
import {renderSceneIdentity} from '@/lib/render-scene-identity';
import {materialProduct,productFinish,finishLabel,type MaterialProduct} from '@/lib/material-products';
import ProductMaterialDialog from './product-material-dialog';
import {setMaterialShortlist} from '@/lib/material-shortlist';
import ObjectPhotoDialog from './object-photo-dialog';
import {createPhotoObject,type ObjectPhotoInput} from '@/lib/object-photo';
...
type Proposal = {
    ...
    title: string;
    details: string;
    source: 'AI' | '빠른 명령' | '스타일' | '사진 팔레트' | '3D 초안' | '디자인 안' | '외관' | '가구 세트' | '배열·정렬' | '소재 교체' | '벽 그리기' | '파티션 개구부' | '브랜드';
    brandSession?:number;
    baseScene?:string;
    addedIds?:string[];
...
async function request(url: string, options?: RequestInit): Promise<any> {
    const r = await (url==='/api/ai'?aiFetch(url,options!):fetch(url,options));
...
export default function InteriorStudio() {
    ...
    const [objectSnapshot,setObjectSnapshot]=useState<{scene:SceneData;session:number}|null>(null);
    const [productSnapshot,setProductSnapshot]=useState<{scene:SceneData;session:number;selection:Selection;mode:'face'|'object'}|null>(null);
    ...
    useEffect(() => {
        request('/api/config').then(d => {setAiReady(d.ai);setImageReady(d.imageReady??d.aiKeyConfigured);setSignedIn(d.signedIn);setAiConfigStatus('ready');}).catch(() => setAiConfigStatus('unavailable'));
    ...
    function openAssemblies(save=false){setProductSaveIds(null);if(proposal){toast('현재 제안을 적용하거나 닫은 뒤 세트를 여세요.');return}engine.current?.cancelTransform();assemblySession.current=projectSession.current;setAssemblySave(save);setMobileLibrary(false);setMobileInspector(false);setModal('assemblies');}
    const brandSnapshot=useRef<{scene:SceneData;session:number}|null>(null);
    function openBrands(){if(proposal||projectLoading)return;engine.current?.cancelTransform();engine.current?.cancelMeasurement();brandSnapshot.current={scene:sceneRef.current,session:projectSession.current};setMobileLibrary(false);setMobileInspector(false);setTool('select');setModal('brands');}
    function previewBrand(candidate:SceneData,title:string,details:string){const snapshot=brandSnapshot.current;if(!snapshot)return;try{assertBrandSnapshot(sceneRef.current,JSON.stringify(snapshot.scene),projectSession.current,snapshot.session);setProposal({scene:validateScene(candidate),title,details,source:'브랜드',baseScene:JSON.stringify(snapshot.scene),brandSession:snapshot.session});setPreview(true);setModal(null);setSelection(null);setEditingGroupId(null);changeSection(null);selectView('top');}catch(e){toast.error((e as Error).message);}}
    ...
    function openObjectPhoto(){if(proposal||projectLoading){toast('현재 제안을 적용하거나 닫은 뒤 물체를 추가하세요.');return;}engine.current?.cancelTransform();engine.current?.cancelMeasurement();setObjectSnapshot({scene:sceneRef.current,session:projectSession.current});setNextModal(null);setMobileLibrary(false);setModal('object-photo');}
    function addPhotoObject(input:ObjectPhotoInput){if(!objectSnapshot||objectSnapshot.session!==projectSession.current||JSON.stringify(objectSnapshot.scene)!==JSON.stringify(sceneRef.current))throw new Error('공간이 변경되었습니다. 물체 사진 만들기를 다시 열어 주세요.');const result=createPhotoObject(sceneRef.current,input);if(!commit(result.scene,'사진 참고 가구를 도면에 추가했습니다. 위치와 치수를 확인하세요.'))return false;setSelection({id:result.node.id,...(result.ids.length>1?{ids:result.ids}:{})});setFaceMode('object');setTool('translate');setModal(null);return true;}
    function openProductMaterials(){if(proposal||projectLoading)return;if(!selectionRef.current){toast('제품 소재를 적용할 벽·바닥·가구를 먼저 선택하세요.');return;}engine.current?.cancelTransform();engine.current?.cancelMeasurement();setProductSnapshot({scene:sceneRef.current,session:projectSession.current,selection:structuredClone(selectionRef.current),mode:faceModeRef.current});setMobileLibrary(false);setModal('product-materials');}
    function updateProductShortlist(ids:string[]){const snapshot=productSnapshot;if(!snapshot||snapshot.session!==projectSession.current||JSON.stringify(snapshot.scene)!==JSON.stringify(sceneRef.current)||modalRef.current!=='product-materials')throw new Error('프로젝트가 변경되었습니다. 소재 라이브러리를 다시 여세요.');const next=setMaterialShortlist(sceneRef.current,ids);if(!commit(next,'프로젝트 마감재 후보를 변경했습니다. 프로젝트 저장 시 함께 보관됩니다.'))return false;setProductSnapshot({...snapshot,scene:sceneRef.current});return true;}
    async function applyProductMaterial(product:MaterialProduct){const snapshot=productSnapshot;if(!snapshot||!snapshot.selection)return false;const viewer=engine.current,ids=selectionIds(snapshot.selection),roomOnly=ids.length===1&&Object.hasOwn(surfaceNames,ids[0]);if(!roomOnly&&!viewer)throw new Error('3D 장면을 불러온 뒤 제품 소재를 적용하세요.');if(!roomOnly)await viewer!.materialNodesReady(ids);if(snapshot.session!==projectSession.current||JSON.stringify(snapshot.scene)!==JSON.stringify(sceneRef.current)||JSON.stringify(snapshot.selection)!==JSON.stringify(selectionRef.current)||snapshot.mode!==faceModeRef.current||modalRef.current!=='product-materials'||!roomOnly&&viewer!==engine.current)throw new Error('장면이나 선택이 변경되었습니다. 제품 라이브러리를 다시 열어 주세요.');const sample:MaterialSample={material:product.base,finish:productFinish(product),sourceName:`${product.brand} ${product.code}`,sourceId:`product:${product.id}`},selection=snapshot.mode==='object'?{...snapshot.selection,face:undefined}:snapshot.selection;const next=pasteMaterial(snapshot.scene,selection,sample,ids.length>1||snapshot.mode==='object'?'object':'face',roomOnly?[]:viewer!.materialCatalog(snapshot.scene));if(next!==snapshot.scene&&!commit(next,`${product.brand} ${product.code} 소재를 적용했습니다.`))return false;setCopiedMaterial({sample,session:snapshot.session});return true;}
    ...
    function openPhotoProducts(){if(proposal||projectLoading){toast('현재 제안을 마친 뒤 제품 목록을 여세요.');return;}engine.current?.cancelTransform();setMobileLibrary(false);setMobileInspector(false);setModal('photo-products');}
    ...
    const commands:StudioCommand[]=[
        ...
        cmd('brand-studio','브랜드 스튜디오 · 표준 모듈','사진과 소재','브랜드별 소재·가구 모듈을 관리하고 빈 상가에 적용합니다.',openBrands,['OFD','올드페리도넛','프랜차이즈','표준','브랜드']),
        cmd('material-presets','내 소재 보관함','사진과 소재','복사한 소재를 보관하고 다른 프로젝트에서도 가져옵니다.',()=>openMaterialPresets(),['프리셋','preset','저장소재','보관함']),
        cmd('photo-object','물체 사진으로 가구 만들기','공간 만들기','실제 물체 사진과 입력 치수로 편집 가능한 가구를 도면에 추가합니다.',openObjectPhoto,['물체','가구사진','디지털화','사진모델']),
        cmd('brand-products','실제 마감재 · 브랜드 제품 코드','사진과 소재','제조사 공식 제품 스와치를 벽·바닥·가구에 적용합니다.',openProductMaterials,['브랜드','제품코드','EGGER','LX','보닥','실제소재']),
        ...
        cmd('photo-products','사진 가구 · 제품 목록','가구 배치','제품 코드와 치수를 확인하고 세트로 보관합니다.',openPhotoProducts,['사진','제품코드','제조사','명세서']),
    ...
    const inspectorContent = <>{materialCopyUi}<LayerControls scene={scene} selection={selection} onAssign={assignSelectionLayer} onManage={openLayers}/><GroupControls scene={scene} selection={selection} editingGroupId={editingGroupId} onCreate={makeGroup} onRename={renameGroup} onUngroup={ungroupSelection} onEdit={enterGroupEdit} onExit={exitGroupEdit}/>{multiple?<BatchInspector scene={scene} selection={selection} onCommit={commit} onSelection={setSelection} onMaterials={showMaterials} onSaveSet={()=>openAssemblies(true)} onPlacement={openPlacement}/>:<><div className="inspector-title"><span className="eyebrow">PROPERTIES</span><h2>{currentName}</h2><span className="selection-label">{node ? kindNames[node.kind] : surface ? '공간 표면' : '선택한 요소 없음'}{selection?.face && faceMode === 'face' ? ' · 선택한 면' : ''}</span>{node?.estimated && <span className="estimate-label">초안 치수 · 현장 확인 필요</span>}</div>{selection && <><div className="property-section"><div className="small-heading"><span>선택 범위</span></div><Tabs value={faceMode} onValueChange={v => {
                ...
                }}><span className={`swatch pattern-${material.pattern}`} style={{ backgroundColor: appearance?.finish.color??material.color }}>{appearance?.finish.catalogId&&materialProduct(appearance.finish.catalogId)?.image&&<img className="catalog-current" src={materialProduct(appearance.finish.catalogId)?.image} alt="적용한 제조사 제품 소재"/>}</span><div><small>현재 소재</small><b>{finishLabel(appearance?.finish??{},material.name)}</b></div><ChevronRight size={17}/></button>}{originalMaterial && <button className="current-material native-material" onClick={() => { setTab('materials'); if (window.innerWidth <= 1050)
 ...
 <div className="property-section"><div className="small-heading"><span>장면 요소</span><button className="scene-layer-link" onClick={openSceneSearch}><Search size={13}/>찾기</button><button className="scene-layer-link" onClick={openLayers}><Layers size={13}/>레이어 {scene.layers?.length??0}</button></div><div className="scene-tree">{Object.entries(surfaceNames).map(([id, name]) => <button className={selection?.id === id ? 'selected' : ''} key={id} onClick={() => selectItem({ id })}><Square size={14}/><span>{name}</span></button>)}<FurnitureTree scene={scene} selection={selection} editingGroupId={editingGroupId} onSelect={selectItem} onSelectGroup={selectWholeGroup} onEdit={enterGroupEdit} onState={setNodeState}/></div></div><button className="text-button full" onClick={openPlacement}>가구 겹침 확인{conflicts.length ? ` · ${conflicts.length}곳` : ``}</button><button className="outline-button full" onClick={openPhotoProducts}><ImagePlus size={15}/>사진 가구 · 제품 목록</button><button className="outline-button full budget-open-button" onClick={openBudget}><Calculator size={15}/>수량·단가·예산 검토</button><button className="outline-button full underlay-open-button" onClick={openUnderlay}><Grid2X2 size={15}/>도면 배경 · 실제 치수로 보정</button><button className="outline-button full room-settings" onClick={() => setModal('room')}><Ruler size={15}/>공간 치수 설정</button></>;
 ...
 <div className="workspace"><SidebarProvider className="studio-sidebar-provider"><Sidebar collapsible="none" className="library"><SidebarContent>{libraryContent}</SidebarContent></Sidebar></SidebarProvider><section className="editor-surface"><div className="editor-bar"><div className="mode-controls"><Tabs value={mode} onValueChange={setMode}><TabsList><TabsTrigger value="easy">간편 편집</TabsTrigger><TabsTrigger value="precise"><Ruler size={14}/>정밀 편집</TabsTrigger></TabsList></Tabs></div><span className="editor-bar-spacer"/><IconButton label="가구·설비 수량과 예산" onClick={openBudget}><Calculator size={17}/></IconButton><IconButton label="레이어 관리 · 가구 분류·표시·잠금" onClick={openLayers}><Layers size={17}/></IconButton><IconButton label="선택한 가구 크게 보기 · F" onClick={()=>focusSelection()}><ScanLine size={17}/></IconButton><IconButton label="여러 가구 선택 · Shift 클릭" active={multiSelect} onClick={()=>{setMultiSelect(!multiSelect);setFaceMode('object')}}><Layers size={17}/></IconButton><button className="outline-button walk-open-button" onClick={openWalk}><Footprints size={16}/>둘러보기</button><IconButton label="줄자 · 치수 목록" onClick={openMeasurements}><Ruler size={17}/></IconButton><IconButton label="간판·어닝·유리 외관 편집" onClick={()=>openFacade()}><Store size={17}/></IconButton><IconButton label="디자인 안 저장·비교" onClick={()=>openReview('variants')}><Copy size={17}/></IconButton><button className="outline-button render-toolbar-button" onClick={openRender}><Sparkles size={15}/>AI 시안</button><IconButton label="실행 취소 · Ctrl Z" onClick={doUndo} disabled={!undo.length}><Undo2 size={17}/></IconButton><IconButton label="다시 실행 · Ctrl Shift Z" onClick={doRedo} disabled={!redo.length}><Redo2 size={17}/></IconButton><span className="toolbar-divider"/><IconButton label="사용 방법" onClick={() => setHelp(true)}><Info size={17}/></IconButton></div><div className="viewport"><SceneCanvas scene={proposal && preview ? proposal.scene : scene} selection={proposal ? null : selection} tool={proposal ? 'select' : tool} view={view} faceMode={faceMode} snap={snap} snapSettings={snapSettings} cutaway={cutaway} section={section} onSelect={selectItem} multiSelect={multiSelect} onTransformGroup={moveGroup} onTransform={updateNode} measurementsVisible={measurementsVisible} onMeasure={saveMeasurement} onMeasureStatus={(started,error)=>{setMeasureStarted(started);setMeasureError(error??'')}} onDrawStatus={setDrawStatus} onDraw={p => {
            ...
            if (window.innerWidth <= 1050)
                ...
                if(proposal.brandSession!==undefined){try{assertBrandSnapshot(sceneRef.current,proposal.baseScene!,projectSession.current,proposal.brandSession);}catch(e){setProposal(null);toast.error((e as Error).message);return;}}
 ...
 {modal==='assemblies'&&<AssembliesDialog key={projectSession.current} scene={scene} ids={productSaveIds??selectedIds.filter(id=>scene.nodes.some(n=>n.id===id))} initialSave={assemblySave} initialName={productSaveIds?scene.nodes.find(n=>n.id===productSaveIds[0])?.objectPhoto?.productName:undefined} onClose={()=>setModal(null)} onPreview={(candidate,ids,name,details,base)=>{if(assemblySession.current!==projectSession.current||base!==JSON.stringify(sceneRef.current)){toast.error('장면이 변경되었습니다. 세트 보관함을 다시 열어 주세요.');setModal(null);return;}setProposal({scene:candidate,title:name,details,source:'가구 세트',baseScene:base,addedIds:ids});setPreview(true);setModal(null);setTool('select');if(view==='front')selectView('perspective');}}/>}
 {modal==='photo-products'&&<PhotoProductsDialog scene={scene} onClose={()=>setModal(null)} onChange={budgetCommit} onSelect={selectBudgetItems} onCreate={()=>openObjectPhoto()} onSave={ids=>{assemblySession.current=projectSession.current;setProductSaveIds(ids);setAssemblySave(true);setModal('assemblies');}}/>}
 ...
 <RenderDialog open={modal === 'render'} onClose={() => setModal(null)} scene={scene} view={view==='front'?'exterior':'interior'} photoUrl={photoUrl} apiKey={apiKey} aiReady={imageReady} onConnect={() => { openAISettings('render'); }} capture={captureForRender} onResult={r => { if (renderSnapshot.current.session !== projectSession.current || renderSnapshot.current.scene !== renderSceneIdentity(sceneRef.current)) {
 ...
 {objectSnapshot&&(modal==='object-photo'||(modal==='settings'&&nextModal==='object-photo'))&&<ObjectPhotoDialog open={modal==='object-photo'} scene={objectSnapshot.scene} apiKey={apiKey} aiReady={aiReady} onConnect={()=>openAISettings('object-photo')} onClose={()=>setModal(null)} onAdd={addPhotoObject}/>}
 {modal==='brands'&&brandSnapshot.current&&<BrandStandardsDialog scene={brandSnapshot.current.scene} onClose={()=>setModal(null)} onPreview={previewBrand}/>}
 {modal==='product-materials'&&productSnapshot&&<ProductMaterialDialog shortlist={scene.materialShortlist??[]} onShortlistChange={updateProductShortlist} currentId={appearance?.finish.catalogId} target={currentName} onClose={()=>setModal(null)} onApply={applyProductMaterial}/>}
 ...
 <Dialog open={modal === 'settings'} onOpenChange={o => !o && closeAISettings()}><DialogContent className="ai-settings-dialog"><DialogHeader><DialogTitle>AI 디자인 연결</DialogTitle><DialogDescription>Hermes로 사진을 분석하고 공간 편집안을 만드세요.</DialogDescription></DialogHeader><HermesConnection onChanged={async()=>{const d=await request('/api/config');setAiReady(d.ai);setImageReady(d.imageReady);setSignedIn(d.signedIn);setAiConfigStatus('ready');}}/>{signedIn===false&&<p role="status" className="fineprint">연결과 AI 분석은 로그인한 사이트에서 사용할 수 있습니다.</p>}<details className="ai-image-connection"><summary>이미지 시안 생성 · OpenAI 연결</summary><p className="fineprint">Hermes 연결은 공간·물체 분석과 편집을 지원합니다. 완성 이미지 생성은 별도 OpenAI 연결이 필요합니다.</p><p className="fineprint">{aiConfigStatus==='unavailable'?'서버 설정을 확인하지 못했습니다.':imageReady?'서버에 이미지 생성 키가 설정되어 있습니다.':apiKey?'현재 세션에 키가 입력되었습니다.':'이미지 생성 연결 전'}</p>{!imageReady&&<><label className="field-label">OpenAI API 키<input className="text-input" type="password" autoComplete="off" value={keyDraft} onChange={e=>setKeyDraft(e.target.value)} placeholder="sk-…"/></label><p className="fineprint">키는 현재 페이지 메모리에서만 사용합니다. 이미지 생성 시 장면과 사진이 OpenAI로 전송되고 API 사용료가 발생합니다. Hermes를 연결하지 않으면 이 키로 사진 분석·문장 편집도 가능합니다.</p><button className="outline-button full" disabled={!keyDraft.trim()} onClick={()=>{setApiKey(keyDraft.trim());setKeyDraft('');toast('세션에 키를 입력했습니다.');}}>현재 세션에 키 사용</button>{apiKey&&<button className="text-button full" onClick={()=>setApiKey('')}>세션 키 지우기</button>}</>}</details><button className="outline-button full" onClick={closeAISettings}>{nextModal?'이전 작업으로 돌아가기':'닫기'}</button></DialogContent></Dialog>
