...
type Proposal = {
    ...
    title: string;
    details: string;
    source: 'AI' | '빠른 명령' | '스타일' | '사진 팔레트';
};
const icons: Record<Kind, typeof Box> = { table: Table2, 'round-table': Circle, chair: Armchair, bench: Sofa, counter: RectangleHorizontal, shelf: Layers, plant: Leaf, pendant: Lightbulb, box: Box, cylinder: Circle, partition: PanelTop, door: DoorOpen, window: Square };
...
function NumberField({ label, value, onCommit, min = -20000, max = 20000, unit = 'mm' }: {
    ...
    max?: number;
    unit?: string;
}) { const [draft, setDraft] = useState(String(Math.round(value))); useEffect(() => setDraft(String(Math.round(value))), [value]); const commit = () => { const n = Number(draft); if (draft.trim() && Number.isFinite(n) && n >= min && n <= max) {
    if (n !== value)
        onCommit(n);
}
else {
...
} }; return <label className="number-field"><span>{label}</span><div><input aria-label={label} type="number" min={min} max={max} value={draft} onChange={e => setDraft(e.target.value)} onBlur={commit} onKeyDown={e => { if (e.key === 'Enter')
    e.currentTarget.blur(); }}/><small>{unit}</small></div></label>; }
function download(data: Blob | string, name: string) { const url = typeof data === 'string' ? data : URL.createObjectURL(data); const a = document.createElement('a'); a.href = url; a.download = name; a.click(); if (typeof data !== 'string')
    setTimeout(() => URL.revokeObjectURL(url), 1000); }
async function request(url: string, options?: RequestInit): Promise<any> { const r = await fetch(url, options); const data = await r.json() as Record<string, any>; if (!r.ok)
    throw new Error(data.error || '작업을 완료하지 못했습니다.'); return data; }
export default function InteriorStudio() {
    const [scene, setScene] = useState<SceneData>(initialScene), [selection, setSelection] = useState<Selection>({ id: 'floor' }), [tab, setTab] = useState('materials'), [filter, setFilter] = useState('전체'), [query, setQuery] = useState(''), [mode, setMode] = useState('easy'), [tool, setTool] = useState<ToolMode>('select'), [view, setView] = useState<ViewMode>('perspective'), [faceMode, setFaceMode] = useState<'face' | 'object'>('face'), [snap, setSnap] = useState(true), [cutaway, setCutaway] = useState(true), [undo, setUndo] = useState<SceneData[]>([]), [redo, setRedo] = useState<SceneData[]>([]), [modal, setModal] = useState<string | null>(null), [mobileLibrary, setMobileLibrary] = useState(false), [mobileInspector, setMobileInspector] = useState(false), [projects, setProjects] = useState<Project[]>([]), [projectId, setProjectId] = useState<string | null>(null), [revision, setRevision] = useState(0), [savedScene, setSavedScene] = useState(''), [saveState, setSaveState] = useState('새 프로젝트'), [busy, setBusy] = useState(false), [uploading, setUploading] = useState(false), [proposal, setProposal] = useState<Proposal | null>(null), [preview, setPreview] = useState(true), [prompt, setPrompt] = useState(''), [aiReady, setAiReady] = useState(false), [apiKey, setApiKey] = useState(''), [keyDraft, setKeyDraft] = useState(''), [aiBusy, setAiBusy] = useState(false), [photoUrl, setPhotoUrl] = useState('/cafe-reference.jpg'), [palette, setPalette] = useState<string[]>([]), [photoImage, setPhotoImage] = useState(''), [projectLoading, setProjectLoading] = useState(false), [pendingOpen, setPendingOpen] = useState<string | null>(null), [help, setHelp] = useState(false);
    ...
    const surface = selection && selection.id in scene.room.surfaces ? scene.room.surfaces[selection.id as keyof SceneData['room']['surfaces']] : undefined;
    const selectedMaterial = surface?.material ?? (selection?.face && node?.faces[selection.face]) ?? node?.material;
    const material = materials.find(m => m.id === selectedMaterial);
    const currentName = node?.name ?? surfaceNames[selection?.id ?? ''] ?? '공간 설정';
    const commit = useCallback((next: SceneData, label?: string) => { try {
        const valid = validateScene(next);
        if (JSON.stringify(valid) === JSON.stringify(sceneRef.current))
            return true;
        setUndo(a => [...a.slice(-99), sceneRef.current]);
        setRedo([]);
        setScene(valid);
        setProposal(null);
    ...
    const updateNode = useCallback((id: string, p: Partial<SceneNode>) => { const s = sceneRef.current; const n = s.nodes.find(n => n.id === id); if (!n)
        return; if (n.locked && Object.keys(p).some(k => k !== 'locked' && k !== 'hidden')) {
        toast.error('잠금을 해제한 뒤 수정하세요.');
        engine.current?.setScene(s);
        return;
    } commit({ ...s, nodes: s.nodes.map(n => n.id === id ? { ...n, ...p } : n) }); }, [commit]);
    function previewLighting(field: 'warmth' | 'intensity', value: number) { if (!lightingStart.current)
        lightingStart.current = sceneRef.current; setScene(s => ({ ...s, lighting: { ...s.lighting, [field]: value } })); setProposal(null); }
    function commitLighting() { if (lightingStart.current) {
    ...
    const doUndo = useCallback(() => { if (!undo.length)
        return; setRedo([...redo, sceneRef.current]); setScene(undo[undo.length - 1]); setUndo(undo.slice(0, -1)); setProposal(null); setSaveState('저장하지 않은 변경'); }, [undo, redo]);
    const doRedo = useCallback(() => { if (!redo.length)
        return; setUndo([...undo, sceneRef.current]); setScene(redo[redo.length - 1]); setRedo(redo.slice(0, -1)); setProposal(null); setSaveState('저장하지 않은 변경'); }, [undo, redo]);
    const removeSelected = useCallback(() => { const id = selectionRef.current?.id, n = sceneRef.current.nodes.find(n => n.id === id); if (!n)
        return; if (n.locked) {
        toast.error('잠긴 요소는 삭제할 수 없습니다.');
        return;
    } commit({ ...sceneRef.current, nodes: sceneRef.current.nodes.filter(n => n.id !== id) }, '요소를 삭제했습니다. 실행 취소로 복원할 수 있습니다.'); setSelection(null); }, [commit]);
    useEffect(() => { request('/api/config').then(d => setAiReady(d.ai)).catch(() => { }); request('/api/projects').then(d => setProjects(d.projects)).catch(() => { }); const raw = localStorage.getItem('spatial-recovery'); if (raw) {
        try {
    ...
    catch { } }, 500); return () => clearTimeout(timer); }, [scene, projectId, revision, savedScene]);
    useEffect(() => { if (scene.photoId) {
    ...
    async function save(asCopy = false) { setBusy(true); const snapshot = sceneRef.current; try {
    ...
    useEffect(() => { const keys = (e: KeyboardEvent) => { const target = e.target as HTMLElement; if (target.closest('input,textarea,[contenteditable=true]'))
        return; if (modal || mobileLibrary || mobileInspector)
        ...
        setTool('rotate'); }; window.addEventListener('keydown', keys); return () => window.removeEventListener('keydown', keys); }, [doUndo, doRedo, removeSelected, modal, mobileLibrary, mobileInspector]);
    function add(kind: Kind) { const id = crypto.randomUUID(), n = createNode(kind, id); if (kind === 'pendant')
        n.y = Math.max(0, scene.room.height - n.height - 300); if (commit({ ...scene, nodes: [...scene.nodes, n] }, `${kindNames[kind]}를 추가했습니다.`)) {
        setSelection({ id });
        setTool('translate');
        setMobileLibrary(false);
    } }
    function paint(id: MaterialId) { if (!selection) {
        toast('먼저 3D 화면에서 벽·바닥·가구를 선택하세요.');
        return;
    } if (surface) {
        commit({ ...scene, room: { ...scene.room, surfaces: { ...scene.room.surfaces, [selection.id]: { material: id } } } });
    }
    else if (node) {
        if (node.locked) {
            toast.error('잠금을 해제한 뒤 소재를 바꾸세요.');
            return;
        }
        const p = selection.face && faceMode === 'face' ? { faces: { ...node.faces, [selection.face]: id }, color: undefined } : { material: id, faces: {}, color: undefined };
        updateNode(node.id, p);
    } }
    function duplicate() { if (!node)
        return; const copy = { ...structuredClone(node), id: crypto.randomUUID(), name: `${node.name} 사본`, x: node.x + 600, locked: false }; if (commit({ ...scene, nodes: [...scene.nodes, copy] }))
        setSelection({ id: copy.id }); }
    async function openProject(id: string) { if (id === 'new') {
    ...
    async function upload(file?: File) { if (!file)
        return; if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type) || file.size > 10 * 1024 * 1024) {
    ...
    async function analyzePalette() { try {
    ...
    async function askAI(withPhoto = false) { const text = withPhoto ? '참고 사진의 인테리어 색감과 소재 컨셉을 현재 3D 공간의 벽, 바닥, 테이블과 조명에 반영해 줘. 치수와 배치는 유지해 줘.' : prompt.trim(); if (!text)
        return; const quick = withPhoto ? null : quickCommands(text, selection); if (quick) {
        try {
    ...
    async function exportFile(type: 'png' | 'glb' | 'json') { if (proposal) {
    ...
    function applyRoom(axis: 'width' | 'depth' | 'height', value: number) { commit({ ...scene, room: { ...scene.room, [axis]: value, source: 'entered' } }); }
