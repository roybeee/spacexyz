...
export default function MaterialTransferControls({sample, canCopy, canPaste, busy, scopeLabel, onCopy, onPaste, onClear, onLibrary, onSavePreset}: {
    ...
    return <section className="material-transfer" aria-label="소재 복사와 붙이기">
        ...
        {sample ? <div className="material-transfer-sample">
            <span className={`material-transfer-chip${sample.textureId ? ' has-texture' : ''}`} style={{backgroundColor:sample.textureId ? undefined : sample.color}} aria-hidden="true">{sample.catalogId&&materialProduct(sample.catalogId)?.image&&<img src={materialProduct(sample.catalogId)?.image} alt="" style={{width:'100%',height:'100%',objectFit:'cover',borderRadius:6}}/>}</span>
